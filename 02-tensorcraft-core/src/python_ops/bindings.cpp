...
struct DeviceArray {
    ...
    DeviceArray(py::array_t<T> arr) { ptr = numpy_to_device(arr, size); }
...
py::array_t<float> py_layernorm(py::array_t<float> input, py::array_t<float> gamma,
    ...
    kernels::layernorm(d_input.ptr, d_gamma.ptr, d_beta.ptr, d_output, batch_size, hidden_size,
                       eps);
    TC_CUDA_CHECK(cudaDeviceSynchronize());
...
py::array_t<float> py_rmsnorm(py::array_t<float> input, py::array_t<float> weight,
    ...
    kernels::rmsnorm(d_input.ptr, d_weight.ptr, d_output, batch_size, hidden_size, eps);
    TC_CUDA_CHECK(cudaDeviceSynchronize());
...
py::array_t<float> py_gemm(py::array_t<float> A, py::array_t<float> B, float alpha = 1.0f,
    ...
    if (version == "naive")
        ver = kernels::GemmVersion::Naive;
    else if (version == "double_buffer")
        ver = kernels::GemmVersion::DoubleBuffer;
