...
RC_GTEST_PROP(ReluTest, Correctness, ()) {
    ...
    hpc::elementwise::relu<float, hpc::elementwise::OptLevel::GridStride>(d_input.data(),
                                                                          d_output.data(), size);
...
TEST(ReluTest, BasicTest) {
    ...
    hpc::elementwise::relu<float, hpc::elementwise::OptLevel::Naive>(d_input.data(),
                                                                     d_output.data(), input.size());
    cudaDeviceSynchronize();
