...
RC_GTEST_PROP(QuantizationTest, RoundTrip, ()) {
    ...
    // Quantize
    hpc::quantization::quantize_int8(d_input.data(), d_quantized.data(), d_scale.data(), rows,
                                     cols);

    // Dequantize
    hpc::quantization::dequantize_int8(d_quantized.data(), d_scale.data(), d_output.data(), rows,
                                       cols);
