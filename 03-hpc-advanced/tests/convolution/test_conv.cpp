...
std::vector<float> reference_conv2d(const std::vector<float>& input,
    ...
    int out_h = output_dim(params.in_height, params.kernel_h, params.pad_h,
                           params.stride_h, params.dilation_h);
    int out_w = output_dim(params.in_width, params.kernel_w, params.pad_w,
                           params.stride_w, params.dilation_w);
                    ...
                    for (int ic = 0; ic < params.in_channels; ++ic) {
                        for (int kh = 0; kh < params.kernel_h; ++kh) {
                            for (int kw = 0; kw < params.kernel_w; ++kw) {
                                int ih = oh * params.stride_h - params.pad_h + kh * params.dilation_h;
                                int iw = ow * params.stride_w - params.pad_w + kw * params.dilation_w;
                                if (ih >= 0 && ih < params.in_height && iw >= 0 && iw < params.in_width) {
                                    int in_idx = b * (params.in_channels * params.in_height * params.in_width)
                                                 + ic * (params.in_height * params.in_width) + ih * params.in_width + iw;
                                    int w_idx = oc * (params.in_channels * params.kernel_h * params.kernel_w)
                                              + ic * (params.kernel_h * params.kernel_w) + kh * params.kernel_w + kw;
                                    sum += input[in_idx] * weight[w_idx];
                                }
                            }
                        }
                    }
                    int out_idx = b * (params.out_channels * out_h * out_w)
...
void run_conv_test(int batch, int in_c, int out_c,
                   int in_h, int in_w,
                   int k_h, int k_w,
    ...
    hpc::convolution::ConvParams params{
        ...
        k_h, k_w, stride, stride, pad, pad, dilation, dilation
    };
