...
public:
    ...
    ProfileResult profile(const std::string& name, int M, int N, int K, int warmup_iters,
                          int bench_iters, KernelFunc kernel, Args... args) {
    ...
    static void print_comparison(const std::vector<ProfileResult>& results,
        ...
        for (const auto& r : results) {
            ...
            printf("║ %-16s │ %10.4f │ %12.2f │ %8.1f GB │ %9.1f%% ║\n", r.name.c_str(),
                   r.avg_time_ms, r.gflops, r.memory_bandwidth_gb, ratio);
        }
    ...
    void analyze() const {
        ...
        for (const auto& p : points_) {
            ...
            printf("║ %-16s │ AI=%.2f │ %.1f/%.1f GFLOPS │ %.1f%% eff │ %s-bound ║\n",
                   p.name.c_str(), p.arithmetic_intensity, p.achieved_gflops, roofline, efficiency,
                   bound);
        }
