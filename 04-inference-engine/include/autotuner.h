...
public:
    ...
    TuningResult tune(int M, int N, int K, cublasHandle_t cublas_handle = nullptr) {
        ...
        std::vector<GemmKernelType> kernel_types = {
            GemmKernelType::NAIVE,
        ...
        for (auto type : kernel_types) {
            ...
            result.time_ms = benchmark_single(type, d_A.get(), d_B.get(), d_C.get(), 
                                              M, N, K, cublas_handle);
    ...
    void execute_best(const float* A, const float* B, float* C,
    ...
    void print_cache() const {
        printf("\n╔═══════════════════════════════════════════════════════════════════════════════╗\n");
        printf("║                           Auto-Tuner Cache                                    ║\n");
        printf("╠═══════════════════════════════════════════════════════════════════════════════╣\n");
        printf("║ %-20s │ %-16s │ %10s │ %10s ║\n", 
               "Dimensions", "Best Kernel", "Time (ms)", "GFLOPS");
        printf("╠═══════════════════════════════════════════════════════════════════════════════╣\n");
...
private:
    ...
    float benchmark_single(GemmKernelType type, const float* A, const float* B, float* C,
    ...
    void execute_kernel(GemmKernelType type, const float* A, const float* B, float* C,
