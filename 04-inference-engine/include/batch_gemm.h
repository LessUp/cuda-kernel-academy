...
struct BatchGemmDesc {
    ...
    BatchGemmDesc(int m, int n, int k, int batch) : M(m), N(n), K(k), batch_size(batch) {
...
inline void launch_batched_gemm_streams(
    const BatchGemmDesc& desc, GemmKernelType kernel_type = GemmKernelType::REGISTER_BLOCKED,
    ...
    for (int i = 0; i < desc.batch_size; i++) {
        ...
        switch (kernel_type) {
            case GemmKernelType::NAIVE:
                launch_naive_matmul(desc.A_ptrs[i], desc.B_ptrs[i], desc.C_ptrs[i], desc.M, desc.N,
                                    desc.K, stream);
                break;
            case GemmKernelType::TILED:
                launch_tiled_gemm(desc.A_ptrs[i], desc.B_ptrs[i], desc.C_ptrs[i], desc.M, desc.N,
                                  desc.K, stream);
                break;
            case GemmKernelType::COALESCED:
                launch_coalesced_gemm(desc.A_ptrs[i], desc.B_ptrs[i], desc.C_ptrs[i], desc.M,
                                      desc.N, desc.K, stream);
                break;
            case GemmKernelType::DOUBLE_BUFFER:
                launch_double_buffer_gemm(desc.A_ptrs[i], desc.B_ptrs[i], desc.C_ptrs[i], desc.M,
                                          desc.N, desc.K, stream);
                break;
            case GemmKernelType::REGISTER_BLOCKED:
                launch_optimized_gemm(desc.A_ptrs[i], desc.B_ptrs[i], desc.C_ptrs[i], desc.M,
                                      desc.N, desc.K, stream);
                break;
...
inline void launch_strided_batched_gemm(
    const float* A, const float* B, float* C, int M, int N, int K, int batch_size,
...
inline void launch_cublas_batched_gemm(cublasHandle_t handle, const float** A_array,
                                       const float** B_array, float** C_array, int M, int N, int K,
                                       int batch_size) {
    const float alpha = 1.0f;
    const float beta = 0.0f;

    // cuBLAS uses column-major, so we compute C^T = B^T * A^T
    CUBLAS_CHECK(cublasSgemmBatched(handle, CUBLAS_OP_N, CUBLAS_OP_N, N, M, K, &alpha, B_array, N,
                                    A_array, K, &beta, C_array, N, batch_size));
}
...
inline BatchPerfStats benchmark_batched_gemm(const BatchGemmDesc& desc, GemmKernelType kernel_type,
