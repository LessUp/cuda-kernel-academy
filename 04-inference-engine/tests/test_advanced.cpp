...
TEST_F(AdvancedTest, MixedPrecisionGemm) {
    ...
    launch_mixed_precision_gemm(reinterpret_cast<half*>(d_A_half.get()),
                                reinterpret_cast<half*>(d_B_half.get()), d_C.get(), M, N, K);
    CUDA_CHECK(cudaDeviceSynchronize());
...
TEST_F(AdvancedTest, StressTestVariousSizes) {
    std::vector<std::tuple<int, int, int>> sizes = {{16, 16, 16},    {33, 65, 17},
                                                    {128, 256, 64},  {257, 513, 129},
                                                    {512, 512, 512}, {1000, 1000, 500}};
