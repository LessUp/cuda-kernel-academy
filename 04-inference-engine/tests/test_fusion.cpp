...
TEST_F(FusionTest, FusedGemmBiasRelu) {
    ...
    cpu_matmul_bias_relu(h_A.data(), h_B.data(), h_C_ref.data(),
                         h_bias.data(), M, N, K, true, true);
    ...
    launch_fused_gemm(d_A.get(), d_B.get(), d_C.get(), d_bias.get(),
                      M, N, K, true, true);
...
TEST_F(FusionTest, FusedGemmReluOnly) {
    ...
    launch_fused_gemm(d_A.get(), d_B.get(), d_C.get(), nullptr,
                      M, N, K, false, true);
    CUDA_CHECK(cudaDeviceSynchronize());
