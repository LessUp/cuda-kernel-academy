...
protected:
    ...
    void TearDown() override { engine_.cleanup(); }
...
TEST_F(InferenceTest, WeightSerializationRoundTrip) {
    ...
    const std::string test_path = make_test_path("test_weights.bin");

    // Create random weights
    std::vector<std::pair<int, int>> layer_dims = {{784, 256}, {256, 128}, {128, 10}};
    ...
    const std::string test_path2 = make_test_path("test_weights2.bin");
...
TEST_F(InferenceTest, MultiLayerForwardPass) {
    ...
    cpu_matmul_bias_relu(input.data(), w1.data(), h1.data(), b1.data(), batch_size, hidden1,
                         in_features, true, true);

    // Layer 2: hidden1 -> hidden2 with ReLU
    cpu_matmul_bias_relu(h1.data(), w2.data(), h2.data(), b2.data(), batch_size, hidden2, hidden1,
                         true, true);

    // Layer 3: hidden2 -> output (no ReLU)
    cpu_matmul_bias_relu(h2.data(), w3.data(), cpu_output.data(), b3.data(), batch_size,
                         out_features, hidden2, true, false);

    // Compare
    float max_error =
        compare_matrices(gpu_output.data(), cpu_output.data(), batch_size * out_features);
