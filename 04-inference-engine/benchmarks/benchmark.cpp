...
void print_header() {
    std::cout << "\n";
    std::cout << "╔══════════════════════════════════════════════════════════════════════════════╗\n";
    std::cout << "║                    Mini-Inference Engine GEMM Benchmark                      ║\n";
    std::cout << "╚══════════════════════════════════════════════════════════════════════════════╝\n";
    std::cout << "\n";
}
...
void print_results(const std::string& name, int M, int N, int K, const PerfStats& stats) {
    std::cout << std::setw(16) << name 
              << " | " << std::setw(8) << std::fixed << std::setprecision(3) << stats.kernel_time_ms << " ms"
              << " | " << std::setw(10) << std::fixed << std::setprecision(2) << stats.gflops << " GFLOPS"
              << " | " << std::setw(8) << std::fixed << std::setprecision(1) << stats.cublas_ratio * 100 << "%"
              << "\n";
}
...
void run_benchmark(int M, int N, int K, cublasHandle_t cublas_handle) {
    ...
    PerfStats cublas_stats = benchmark_kernel(GemmKernelType::CUBLAS, 
        d_A.get(), d_B.get(), d_C.get(), M, N, K, 10, 50, cublas_handle);
    ...
    for (const auto& [type, name] : kernels) {
        PerfStats stats = benchmark_kernel(type, d_A.get(), d_B.get(), d_C.get(), 
                                           M, N, K, 10, 50, cublas_handle);
...
void verify_correctness(int M, int N, int K) {
    ...
    for (const auto& [type, name] : kernels) {
        ...
        std::cout << "│ " << std::setw(14) << name << ": max error = " 
