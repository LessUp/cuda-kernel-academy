...
flowchart LR
    ...
    Tiled -->|"1.75×"| Coalesced["Coalesced<br/>4.5 TFLOPS"]
...
```mermaid
flowchart TB
    subgraph A_mat["Matrix A (M × K)"]
        direction LR
        A1["T1"] --- A2["T2"] --- A3["T3"]
    end

    subgraph B_mat["Matrix B (K × N)"]
        direction LR
        B1["T1"] --- B2["T2"] --- B3["T3"]
    end

    subgraph C_mat["Matrix C (M × N)"]
        C1["每个 Block<br/>处理一个<br/>TILE × TILE<br/>输出块"]
    end
...
```mermaid
flowchart LR
    subgraph W["Without Double Buffering"]
        W1["Load T1"] --> W2["Compute T1"] --> W3["Load T2"] --> W4["Compute T2"]
    end

    subgraph D["With Double Buffering"]
...
flowchart TB
    subgraph TC["Tensor Core"]
        A_tc["A (16 × 16, FP16)"] --> M["16 × 16 × 16<br/>MMA"]
        B_tc["B (16 × 16, FP16)"] --> M
        M --> C_tc["C (16 × 16, FP32)"]
    end
